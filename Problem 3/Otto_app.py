...
class MainWindow(qtw.QWidget, Ui_Form):
    def __init__(self):
        ...
        # 'someWidgets' needs to be self.somewidgets, so I created a shortcut for this.
        self.someWidgets = someWidgets
    ...
    def OttoOrDieselSelected(self):
        """
        I have to make a function that will change the output based on whether the Otto cycle
        or Diesel cycle is selected, then output the graph appropriately for the cycle chosen
        by the user.
        """
        # If Diesel cycle is selected (which has the index of 1), update the controller widgets and title
        if self.comboBox.currentIndex() == 1:
            self.controller=dieselCycleController()
            self.controller.setWidgets(w=self.someWidgets)
            self.gb_Input.setTitle("Diesel Cycle")
        # Otherwise, the Otto cycle is selected and I need to update the controller widgets and title
        else:
            self.controller=ottoCycleController()
            self.controller.setWidgets(w=self.someWidgets)
            self.gb_Input.setTitle("Otto Cycle")

#if this module is being imported, this won't run. If it is the main module, it will run.
if __name__== '__main__':
    app = qtw.QApplication(sys.argv)
    mw = MainWindow()
    mw.setWindowTitle('Otto or Diesel Cycle Calculator')
    sys.exit(app.exec())